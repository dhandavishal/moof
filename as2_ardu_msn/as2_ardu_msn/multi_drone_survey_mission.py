    ...
    def state_callback(self, msg):
        ...
        if self.mavros_state is None or self.mavros_state.armed != msg.armed:
            self.node.get_logger().info(
                f"[{self.namespace}] State - Armed: {msg.armed}, Mode: {msg.mode}"
            )
    ...
    def wait_for_ready(self, timeout_sec=30.0):
        ...
        while (time.time() - start_time) < timeout_sec:
            ...
            if self.mavros_state is None:
                self.node.get_logger().info(f"[{self.namespace}] No MAVROS state received yet...")
            elif not self.mavros_state.connected:
                self.node.get_logger().info(f"[{self.namespace}] MAVROS not connected to FCU...")
            else:
                self.node.get_logger().info(f"[{self.namespace}] Ready - Connected: {self.mavros_state.connected}")
                return True
            
            rclpy.spin_once(self.node, timeout_sec=0.1)
            time.sleep(0.5)  # Increased sleep for less spam
        
        self.node.get_logger().error(f"[{self.namespace}] Readiness timeout after {timeout_sec}s")
        return False
