...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        # Include MAVROS APM launch (which works!)
        IncludeLaunchDescription(
            ...
            launch_arguments={
                ...
                'tgt_system': '1',
                'tgt_component': '1',
                # Remove namespace to match working test configuration
            }.items()
        ),
        
        # Add Aerostack2 platform
        Node(
            ...
            parameters=[
                {
                    ...
                    'global_frame': 'earth',
                    'odom_frame': 'odom',
                    'mavros_namespace': '',  # Empty namespace since mavros is at root level
                    'control_modes_file': os.path.join(pkg_dir, 'config', 'control_modes.yaml'),
                    'external_odom': True,  # Add missing parameter
                }
            ],
            output='screen'
        ),
        ...
        Node(
            ...
            parameters=[{
                ...
                'base_frame': f'{drone_namespace}/base_link',
                'global_frame': 'earth',
                'odom_frame': f'{drone_namespace}/odom',
                'enable_tf': True,  # Add missing parameter
            }],
            output='screen'
        ),
        ...
        Node(
            ...
            namespace=drone_namespace,
            parameters=[{
                'controller_plugin_name': 'pid_speed_controller::Plugin',
                'cmd_freq': 30.0,
                'info_freq': 10.0,
                'motion_reference_adder_plugin_name': 'position_motion',
                'plugin_config_file': os.path.join(pkg_dir, 'config', 'control_modes.yaml'),
            }],
            output='screen'
        ),
    ])